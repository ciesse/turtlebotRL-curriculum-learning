...
class TurtleBotEnv(gym.Env, Node):
    def __init__(self):
        ...
        self.world_sdf_name = "medium.sdf"
    ...
    def step(self, action):
        ...
        # REWARD SHAPING  (Bilanciato per modello Medium)     

        obstacle_reward = 0.0
        danger_zone = 0.35      # Il robot inizia a preoccuparsi a 35 cm
        death_zone = 0.13       # Il robot muore a 13 cm
        if min_distance < danger_zone and min_distance > death_zone:
            # Calcola quanto il robot è "penetrato" nella bolla di pericolo (da 0.0 a 1.0)
            penetration = (danger_zone - min_distance) / (danger_zone - death_zone)
            
            
            # Moltiplicatore a 10.0 per incentivare il robot a evitare la zona di pericolo, ma non troppo severo da causare panico
            obstacle_reward = -10.0 * (penetration ** 2)
        ...
        reward = distance_reward + obstacle_reward + time_penalty

        # Punizione Anti-Suicidio (Ridotta a -50.0 per evitare il terrore iniziale)
