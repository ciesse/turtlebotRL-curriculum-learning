...
class TurtleBotEnv(gym.Env, Node):
    def __init__(self):
        ...
        self.world_sdf_name = "forest.sdf"  
    ...
    # STEP CONTINUO E REWARD
    def step(self, action):
        twist = Twist()

        # Mappatura Azione Continua: Lineare [-0.05 -> 0.22] | Angolare [-1.0 -> 1.0]
        # INSERITA LA MICRO-RETROMARCIA 
        raw_linear = float(action[0])
        linear_mapped = (raw_linear + 1.0) / 2.0 * 0.27 - 0.05
        twist.linear.x = float(np.clip(linear_mapped, -0.05, 0.22))
        ...
        # REWARD SHAPING V3 (Ottimizzato per Spazi Stretti)    

        # 1. Penalità Ostacoli (meno terrore per i passaggi stretti)
        obstacle_reward = 0.0
        danger_zone = 0.28      # Ridotto da 0.35
        death_zone = 0.13       
        if min_distance < danger_zone and min_distance > death_zone:
            penetration = (danger_zone - min_distance) / (danger_zone - death_zone)
            # Moltiplicatore a -4.0 (invece di -10.0)
            obstacle_reward = -4.0 * (penetration ** 2)

        # 2. Reward di avvicinamento al Goal
        distance_reward = (self.last_distance - distance_to_goal) * 5.0
        self.last_distance = distance_to_goal

        # 3. Penalità di Tempo
        time_penalty = -0.05

        # 4. Anti-Stallo Semplice (Sui comandi di velocità)
        stall_penalty = 0.0
        if abs(twist.linear.x) < 0.02 and self.step_count > 20:
            stall_penalty = -0.20
            
        # 5. Wobble Penalty (Per evitare zig-zag)
        wobble_penalty = -0.02 * abs(action[1])
        
        # Reward Totale Combinato
        reward = distance_reward + obstacle_reward + time_penalty + stall_penalty + wobble_penalty

        
        # CONDIZIONI DI FINE EPISODI
        # Punizione Collisione
    ...
    def _respawn_robot_at_origin(self):
        ...
        # SPAWN CASUALE E ROTAZIONE CASUALE
        spawn_points = [
            
            (1.8, 1.8),       # Angolo Top-Right
            (-1.8, 1.8),      # Angolo Top-Left
            (1.8, -1.8),      # Angolo Bottom-Right
            (-1.8, -1.8)      # Angolo Bottom-Left
        ]
        idx = np.random.randint(0, len(spawn_points))
        spawn_x, spawn_y = spawn_points[idx]
        
        # Genera un angolo casuale da -180° a +180° per costringerlo a girarsi
        spawn_yaw = np.random.uniform(-math.pi, math.pi)
        
        # Conversione per Gazebo (Yaw -> Quaternione)
        qz = math.sin(spawn_yaw / 2.0)
        qw = math.cos(spawn_yaw / 2.0)

        req_spawn = f'sdf_filename: "{model_path}", name: "burger", pose: {{ position: {{ x: {spawn_x}, y: {spawn_y}, z: 0.05 }}, orientation: {{ x: 0.0, y: 0.0, z: {qz}, w: {qw} }} }}'
        ...
        if not self._pose_received.wait(timeout=2.0):
            with self._pose_lock:
                self.robot_x, self.robot_y, self.robot_yaw = spawn_x, spawn_y, spawn_yaw
            self.get_logger().warn(f"Posa non ricevuta. Uso fallback ({spawn_x},{spawn_y}).")

        robot_x, robot_y, robot_yaw = self._get_pose()
        self.get_logger().info(f"Nuovo Spawn Casuale | pose=({robot_x:.2f}, {robot_y:.2f}, yaw={robot_yaw:.2f})")
    ...
    def reset(self, seed=None, options=None):
        ...
        # Genera SEMPRE un nuovo goal, forzando la massima esplorazione
