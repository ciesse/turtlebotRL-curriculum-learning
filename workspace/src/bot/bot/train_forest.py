...
def find_latest_checkpoint(models_dir):
    ...
    checkpoints = [
        ...
        if f.startswith("turtlebot_forest") and f.endswith(".zip")
    ]
...
def main():
    ...
    print("Inizializzazione ambiente PPO Continuo (Forest da Medium)...")

    models_dir = "./modelli_salvati_forest/"
    tensorboard_dir = "./ppo_tensorboard/"  

    os.makedirs(models_dir, exist_ok=True)
    os.makedirs(tensorboard_dir, exist_ok=True)

    stats_file = os.path.join(models_dir, "training_stats_forest.json")
    ...
    if ultimo_checkpoint is None:
        #TRANSFER LEARNING: Partiamo dal Medium finale
        modello_base_path = "./modelli_salvati_medium_cont_random/turtlebot_medium_cont_random_525000_steps.zip"
        
        if os.path.exists(modello_base_path):
            print(f"\n[TRANSFER LEARNING] Caricamento cervello base: {modello_base_path}\n")
            
            # ACCESA L'ESPLORAZIONE E IL LEARNING RATE
            nuovi_parametri_tl = {
    ...
    print(f"Avvio addestramento Forest")
    ...
    try:
        model.learn(
            ...
            tb_log_name="PPO_Forest"  
        )

        final_path = os.path.join(models_dir, "turtlebot_forest_final")
        model.save(final_path)
        print(f"Addestramento Forest COMPLETATO! Modello finale salvato in: {final_path}")
    
    except KeyboardInterrupt:
        interrupted_path = os.path.join(models_dir, "turtlebot_forest_INTERRUPTED")
